...
def is_human_like(min_x, max_x, min_y, max_y, num_points):
    ...
    # Too small (noise or distant object) - RELAXED
    if width < 30 or height < 30:
        print("Reject: too small {}x{}".format(width, height))
        return False
    
    # Too large (whole scene movement)
    if width > 250 or height > 200:
        print("Reject: too large {}x{}".format(width, height))
        return False
    
    # Human-like aspect ratio - RELAXED to allow faces and torsos
    aspect_ratio = height / width if width > 0 else 0
    if aspect_ratio < 0.5 or aspect_ratio > 4.0:
        print("Reject: bad aspect {:.2f}".format(aspect_ratio))
        return False
    
    # Should have sufficient motion points (not just a corner)
    if num_points < 2:  # RELAXED from 3 to 2
        print("Reject: too few points {}".format(num_points))
        return False
    
    # Position check - REMOVED, humans can be anywhere
    
    print("ACCEPTED: {}x{} aspect={:.2f} pts={}".format(width, height, aspect_ratio, num_points))
    return True
...
while True:
    ...
    if frame_count > 20:
        ...
        if clusters:
            print("Found {} clusters with {} total motion points".format(len(clusters), len(motion_points)))
